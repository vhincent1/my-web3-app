...
const run = async () => {
  const options: any = {
    admin_key: { type: 'string', desc: 'keypair file', value: appConfig.ADMIN_WALLET.filePath },
    program_key: { type: 'string', desc: 'keypair file', value: appConfig.PROGRAM_WALLET.filePath },
    port: { alias: 'p', type: 'number', desc: 'client port', value: `${appConfig.PORT}` },
  };
  ...
  try {
    ...
    if (flags.admin_key) appConfig.ADMIN_WALLET = keypairUtils.loadKeypair(flags.admin_key, true);
    if (flags.program_key) appConfig.PROGRAM_WALLET = keypairUtils.loadKeypair(flags.program_key, true);
